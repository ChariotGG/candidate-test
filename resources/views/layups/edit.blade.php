    ...
    <div class="py-8 bg-gray-50 dark:bg-gray-900 min-h-screen">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 text-sm text-gray-500 dark:text-gray-400">
                ...
                <span class="mx-2">/</span>
                <a href="{{ route('layups.show', $layup) }}" class="hover:text-gray-900 dark:hover:text-gray-100">{{ $layup->name }}</a>
                <span class="mx-2">/</span>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white" style="font-family: ui-serif, Georgia, serif;">Edit Layup</h1>
                    <p class="text-gray-500 dark:text-gray-400 text-sm mt-1">
                        Supplier: <span class="font-medium text-gray-700 dark:text-gray-300">{{ $layup->supplier->name }}</span>
                    </p>
                </div>

                <form action="{{ route('layups.update', $layup) }}" method="POST" class="p-6 space-y-6">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                            ...
                            <div class="text-2xl font-bold text-[#2D6A4F]">{{ $layup->layers->count() }}</div>
                        </div>
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 border border-gray-200 dark:border-gray-600">
                            <div class="text-xs text-gray-400 font-bold uppercase tracking-wider mb-1">Total Thickness</div>
                            <div class="text-2xl font-bold text-[#2D6A4F]">{{ $layup->layers->sum('thickness') }}mm</div>
                        </div>
                    </div>
